...
class KalmanFilter(object):
    ...
    # this list is used to store the a apriori estimates xk^{-} starting from x1^{-}
        ...
        self.estimationErrorCovarianceMatricesAposteriori=np.asarray(P0).reshape((1,3,3))
    ...
    def propagateDynamics(self,inputValue):
        ...
        Pk_minus=self.A*np.asmatrix(self.estimationErrorCovarianceMatricesAposteriori[self.currentTimeStep,:,:])*(self.A.T)+self.Q

        if self.currentTimeStep==0:
            self.estimates_apriori=np.asarray(xk_minus).reshape(3,1)
            self.estimationErrorCovarianceMatricesApriori=np.asarray(Pk_minus).reshape((1,3,3))
        else:
            self.estimates_apriori=np.hstack((self.estimates_apriori,np.asarray(xk_minus).reshape(3,1)))
            self.estimationErrorCovarianceMatricesApriori=np.vstack((self.estimationErrorCovarianceMatricesApriori,np.asarray(Pk_minus).reshape((1,3,3))))
    ...
    def computeAposterioriEstimate(self,currentMeasurement):
        # gain matrix
        Kk=np.asmatrix(self.estimationErrorCovarianceMatricesApriori[self.currentTimeStep-1,:,:])*(self.C.T)*np.linalg.inv(self.R+self.C*np.asmatrix(self.estimationErrorCovarianceMatricesApriori[self.currentTimeStep-1,:,:])*(self.C.T))
        
        # prediction error
        error_k=currentMeasurement.reshape(3,1)-self.C*self.estimates_apriori[:,self.currentTimeStep-1].reshape(3,1)
        # a posteriori estimate
        xk_plus=self.estimates_apriori[:,self.currentTimeStep-1].reshape(3,1)+Kk*error_k
        ...
        if self.currentTimeStep==1:
            self.errors=np.asarray(error_k).reshape(3,1)
            self.gainMatrices=np.asarray(Kk).reshape((1,3,3))
        else:
            self.errors=np.vstack((self.errors,np.asarray(error_k).reshape(3,1)))
            self.gainMatrices=np.vstack((self.gainMatrices,np.asarray(Kk).reshape((1,3,3))))
        self.estimates_aposteriori=np.hstack((self.estimates_aposteriori,np.asarray(xk_plus)))
        self.estimationErrorCovarianceMatricesAposteriori=np.vstack((self.estimationErrorCovarianceMatricesAposteriori,np.asarray(Pk_plus).reshape(1,3,3)))
